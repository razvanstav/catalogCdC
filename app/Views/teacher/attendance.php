...
<section class="card card--flat">
  <form action="/teacher/attendance" method="GET" class="form-grid attendance-filters">
    <div class="form-group">
      <label class="form-label" for="group_id">Grupă sau curs</label>
    ...
    <div class="form-group">
      <label class="form-label" for="lesson_id">Ședință</label>
      <select id="lesson_id" name="lesson_id" class="form-control" data-submit-on-change>
        <?php foreach ($lessons as $lesson): ?>
          <option value="<?= e($lesson['id']) ?>" <?= $lesson['id'] === $selectedLessonId ? 'selected' : '' ?>><?= e($lesson['title']) ?> • <?= format_date_ro($lesson['lesson_date']) ?> (<?= e($lesson['start_time']) ?>–<?= e($lesson['end_time']) ?>)</option>
        <?php endforeach; ?>
        <?php if (empty($lessons)): ?><option value="">Nicio ședință programată</option><?php endif; ?>
      </select>
    </div>
  </form>
</section>
...
<form action="/teacher/attendance" method="POST" class="attendance-form">
  ...
  <section class="card card--flush">
    <div class="panel-toolbar">
      ...
      <button type="submit" class="btn btn--sage btn--sm" <?= !$selectedLessonId ? 'disabled' : '' ?>>Salvează prezența</button>
    </div>
    ...
    <?php else: ?>
      <div class="empty-state">
        <div>
          ...
          <div class="empty-state__title">Nu există elevi în această grupă sau ședință</div>
          <p class="empty-state__text">Înscrie elevi în grupă sau adaugă un elev venit la recuperare folosind butonul de sus.</p>
        </div>
      </div>
    <?php endif; ?>
  </section>
